...
class Camera:
    ...
    def get_path(self,img,canny_lower_threshold,canny_upper_threshold,dilate_iteration):
        ...
        gradient = remove_small_area(gradient_tab,5000)
        external_maze = remove_small_area(external_maze,5000)
...
if __name__ == '__main__':
    cam = Camera(0)
    im_in = cv2.imread('images/process_data/test.png')
    canny_lower_threshold,canny_upper_threshold,dilate_iteration = cam.image_tuning(im_in)
    cam.get_path(im_in,canny_lower_threshold,canny_upper_threshold,dilate_iteration)